...
import { Button, CircularProgress, Stack, TableBody } from '@mui/material';
...
import { columns } from '../../../../../../models/type';
...
export default function Tables() {
  ...
  const getEmployee = React.useCallback(async (page: number = 1, valueSearch: string = '') => {
    setLoading(true);
    const json = await axios.get(`${API_PATHS.employee}?page=${page}&search=${valueSearch}`, {
      headers: {
          ...
          {loading ? (
            <TableBody>
              <TableRow>
                <TableCell>
                  <CircularProgress
                    sx={{ position: 'absolute', top: '68%', left: '60%', transform: 'transalateY(-50%)' }}
                  />
                </TableCell>
              </TableRow>
            </TableBody>
